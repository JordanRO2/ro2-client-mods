// Char_TexAni_Dye MAIN LIT pixel shader — TOON/REALISTIC live-tunable variant.
// PS-splice only (VS blobs kept byte-identical -> GPU skinning intact). Global param NAMES
// match the char effect globals; sampler REGISTERS match this shader's ASM:
...
float4 TextureAnimationValue;
sampler2D BaseSampler : register(s0);   // s0  base map
sampler2D GlowSampler : register(s1);   // s1  glow/anim map
sampler2D ToonSampler : register(s2);   // s2  toon ramp
...
PS_OUT main(PS_IN i)
{
    ...
    // toon lighting term from vertex SH colour
    ...
    // toon ramp + animated glow map
    float2 toonUV = ndl * 0.5 + 0.5;
    float4 toon = tex2D(ToonSampler, toonUV);
    float2 glowUV = i.texcoord2.xy + TextureAnimationValue.xy;
    ...
    // glow blends the dye/weave colour into the base
    float invGlow = -glow.x + 1.0;
    float3 weaveCol = base.xyz * glow.x * Extra_WeaveColor.xyz;
    float3 baseMix = base.xyz * invGlow + weaveCol;
