...
struct PS_IN {
    float4 texcoord  : TEXCOORD0;   // t0: .xyz = +viewDir, surface->eye. NOT -viewDir: the old
                                    // comment here was FALSE and is load-bearing now. Stock
                                    // proves +V via `add r5,t0,c8` (H=V+L) and
                                    // `dp3_sat r1.x, c8, -t0` (the explicit negate is only
                                    // needed if t0 is +V). 'Correcting' the code to match the
                                    // old comment pins the fresnel at its 2.0 cap on every
                                    // pixel and doubles the whole Hair highlight. .w = fog
